...
&emsp;&emsp;As Figuras 1 e 2 oferecem uma visualização em barplot e em radar chart consolidando essas métricas:
...
&emsp;&emsp;Na fase de transfer learning (A08), foi explorado sistematicamente o impacto de 7 estratégias de data augmentation na convergência e desempenho do modelo MobileNetV2. As análises de histogramas (Figuras 6 e 7) sintetizam os resultados obtidos:
...
&emsp;&emsp;No estágio final de otimização (A08), foi conduzido grid search sobre learning rate e batch size durante a fase de fine-tuning parcial do MobileNetV2. Os resultados são visualizados em mapas de calor (Figuras 8, 9 e 10) que indicam desempenho em função de LR e BS:
...
&emsp;&emsp;A Figura 11 compara as matrizes de confusão dos três modelos em seus conjuntos de validação/teste. As matrizes normalizadas revelam a proporção de erros em cada classe, permitindo identificar vieses de classificação:
...
&emsp;&emsp;Complementando a avaliação quantitativa, a Figura 14 apresenta mapas de ativação Grad-CAM que visualizam quais regiões e canais espectrais o modelo MobileNetV2 prioriza ao fazer predições. Esse mecanismo de interpretabilidade revela se o modelo aprendeu padrões geológicos significativos:
...
**Análise de Erros — Falsos Positivos vs. Verdadeiros Positivos:** A Figura 16 visualiza mapas Grad-CAM para amostras que o modelo classificou incorretamente, revelando ambiguidade geológica:
...
&emsp;&emsp;Além da avaliação por chip individual, as Figuras 17, 18 e 19 apresentam análises espaciais que mostram a distribuição geográfica de scores de prospectividade do modelo MobileNetV2, oferecendo perspectiva agregada útil para planejamento de exploração:
...
1. **Priorização de Alvos:** Regiões com alta densidade de altos scores (Figura 17) indicam áreas onde múltiplos chips adjacentes apresentam assinatura espectral consistente com REE, reduzindo risco de falso alerta isolado.

2. **Detecção de Padrões Regionais:** A comparação entre TP e FP geograficamente (Figura 18) pode revelar se erros concentram-se em províncias geológicas específicas, sugerindo dominância de classes espectrais similares.

3. **Calibração Operacional:** Caso exploração de altos clusters (Figura 19) resulte em maior taxa de confirmação, pode-se ajustar threshold de decisão dinamicamente por região geográfica.
...
&emsp;&emsp;Apesar dos resultados encorajadores, algumas limitações devem ser consideradas. O dataset de 295 amostras situa-se no limiar inferior de viabilidade para deep learning, e a validação foi conduzida com rótulos de referência de escritório, sem confirmação de campo para o modelo final. Trabalhos futuros podem explorar arquiteturas mais profundas, integração multissensor e validação geológica in situ. Ainda assim, o pipeline desenvolvido demonstra o potencial do uso de aprendizado profundo aplicado a dados de sensoriamento remoto como ferramenta de apoio à prospecção mineral, permitindo ordenar áreas de interesse com base em escores probabilísticos de potencial prospectivo.
...
&emsp;&emsp;A Figura 22 apresenta a matriz de confusão do modelo final no conjunto de teste (n=59). O modelo classificou corretamente 52 das 59 amostras (88,14%), com apenas 4 falsos positivos e 3 falsos negativos.
...
&emsp;&emsp;A Figura 23 apresenta as curvas ROC e Precision-Recall do modelo final, que sintetizam o desempenho discriminativo em diferentes limiares de decisão.
...
| **Figura 22** | TN=32, FP=4, FN=3, TP=20 | O modelo final equilibra detecção de áreas promissoras e controle de alarmes indevidos |
| **Figura 23** | ROC-AUC=0,9167; PR-AUC=0,8588 | O desempenho permanece robusto sob diferentes limiares de decisão |
| **Figura 14** | Ativações coerentes via Grad-CAM | As predições possuem interpretabilidade e plausibilidade geológica |
...
&emsp;&emsp;O avanço seguinte, obtido com transfer learning e adaptação espectral da MobileNetV2 no artefato A08, representou um salto adicional de desempenho, alcançando 84,75% de acurácia, F1-score de 0,8085 e ROC-AUC de 0,9312. Esse resultado mostrou que a reutilização de representações pré-aprendidas, aliada à projeção das 9 bandas ASTER para uma entrada compatível com pesos ImageNet, foi capaz de capturar padrões mais complexos mesmo em um cenário de baixa disponibilidade de dados rotulados. Finalmente, o pipeline integrado e reprodutível do A11 consolidou o melhor equilíbrio global entre as métricas avaliadas, atingindo **88,14% de acurácia, 83,33% de precisão, 86,96% de recall, F1-score de 0,8511, acurácia balanceada de 87,92%, ROC-AUC de 0,9167 e PR-AUC de 0,8588**, em um conjunto de teste com 59 amostras. Esses números confirmam que a solução final não apenas superou os modelos anteriores em desempenho geral, mas também apresentou comportamento mais estável e operacionalmente mais útil.

&emsp;&emsp;A Tabela 8 é particularmente importante porque não mostra apenas um melhor resultado isolado, mas sim uma progressão coerente entre abordagens de complexidade crescente. Essa progressão sustenta empiricamente a hipótese central do trabalho: a tarefa de classificação de potencialidade para Elementos de Terras Raras em imagens multiespectrais ASTER se beneficia diretamente da incorporação de informação espacial, de arquiteturas convolucionais mais expressivas e do uso de transfer learning em contextos de amostras limitadas. Em outras palavras, os ganhos observados não são aleatórios nem pontuais; eles refletem um refinamento cumulativo da representação dos dados, da arquitetura de modelagem e da estratégia experimental.

&emsp;&emsp;No eixo de análise de erro, a Figura 22 reforça essa interpretação ao explicitar a distribuição dos acertos e erros do modelo final. No conjunto de teste, a matriz de confusão indica **32 verdadeiros negativos, 20 verdadeiros positivos, 4 falsos positivos e 3 falsos negativos**, totalizando 52 classificações corretas em 59 amostras. Esse resultado é altamente relevante do ponto de vista aplicado, pois demonstra que o modelo final alcança simultaneamente boa capacidade de detecção e controle de alarmes indevidos. Em termos práticos, os 4 falsos positivos correspondem a áreas que poderiam ser priorizadas para investigação adicional sem necessariamente conter mineralização de interesse, enquanto os 3 falsos negativos representam oportunidades potenciais que deixariam de ser sinalizadas. O fato de esses valores terem sido reduzidos em relação às etapas anteriores indica amadurecimento da generalização do pipeline e maior adequação a cenários reais de apoio à prospecção.

&emsp;&emsp;Complementarmente, a Figura 23 oferece evidência de que o bom desempenho do modelo final não depende de um único limiar de decisão. A **ROC-AUC de 0,9167** atesta excelente separabilidade entre classes positiva e negativa ao longo de diferentes thresholds, enquanto a **PR-AUC de 0,8588** mostra manutenção de precisão elevada mesmo em regimes de alto recall. Essa leitura é especialmente importante no contexto de exploração mineral, em que o custo operacional dos erros não é simétrico: um falso negativo pode significar a perda de uma oportunidade de prospecção, ao passo que um falso positivo implica direcionar esforço de campo para uma área posteriormente descartada. A análise conjunta das curvas ROC e Precision-Recall mostra que o modelo pode ser calibrado para diferentes estratégias de uso — desde um modo mais conservador, priorizando precisão, até um modo mais exploratório, priorizando sensibilidade — sem perda abrupta de qualidade preditiva.

&emsp;&emsp;Do ponto de vista qualitativo e interpretativo, a Figura 14 adiciona uma camada essencial de evidência ao demonstrar que o modelo não opera como uma caixa-preta completamente opaca. As visualizações por Grad-CAM indicam que, nas amostras classificadas como positivas, as regiões de maior ativação tendem a se concentrar em estruturas locais mais intensas e organizadas, enquanto, nos casos negativos, a resposta do modelo aparece de forma mais difusa ou menos focalizada. Essa diferença de padrão sugere que a rede está, de fato, respondendo a combinações espaciais e espectrais relevantes para a tarefa, e não apenas a ruído ou artefatos aleatórios da imagem. Assim, a interpretabilidade obtida complementa os bons indicadores quantitativos e fortalece a credibilidade científica do pipeline desenvolvido.

&emsp;&emsp;Outro aspecto que merece destaque é o caráter **integrado, reprodutível e auditável** do resultado final. O artefato A11 não se restringe a um modelo isolado com boa performance, mas organiza todo o fluxo de execução — preparação dos dados, particionamento, treinamento em duas fases, inferência, avaliação e exportação de métricas e visualizações — em um pipeline com configuração versionada e seed fixa. Esse ponto é particularmente relevante em ciência de dados aplicada, pois garante que os resultados reportados possam ser reproduzidos, verificados e estendidos por outros pesquisadores ou por equipes técnicas interessadas em adaptar a solução a novos contextos geológicos.

&emsp;&emsp;Também é importante registrar que a leitura final das evidências deve ser feita de maneira criteriosa e honesta. Embora o A11 tenha apresentado o melhor conjunto global de métricas, observa-se que sua ROC-AUC final (0,9167) é ligeiramente inferior àquela obtida na etapa de grid search do A08 (0,9312). Ainda assim, essa diferença não invalida a superioridade do pipeline final, pois todas as demais métricas principais — acurácia, F1-score, acurácia balanceada e contagem de erros concretos na matriz de confusão — evoluíram positivamente. Isso sugere que o modelo final se mostrou mais equilibrado e mais ajustado ao cenário operacional do projeto, mesmo sob condições rigorosas de reprodutibilidade e treinamento controlado.

&emsp;&emsp;Dessa forma, o conjunto formado pela **Tabela 8**, pela **Figura 22**, pela **Figura 23** e pela **Figura 14** sustenta de maneira convergente as conclusões do estudo. Em síntese, as evidências reunidas indicam que: **(i)** o uso de informação espacial é determinante para superar abordagens puramente tabulares; **(ii)** o transfer learning com adaptação espectral é uma estratégia eficaz para imagens ASTER multibanda em cenários com poucos dados; **(iii)** o pipeline final apresenta desempenho suficientemente robusto para apoiar a triagem inicial de áreas com potencial para REE; e **(iv)** a combinação entre métricas quantitativas e análise interpretável fortalece a utilidade prática e a legitimidade científica da proposta. Assim, o trabalho não apenas demonstra viabilidade técnica, mas também entrega uma base metodológica consistente para futuras extensões em prospecção mineral assistida por inteligência artificial e sensoriamento remoto.
