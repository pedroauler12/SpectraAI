...
&emsp;&emsp; Historicamente, aplicações de aprendizado de máquina em sensoriamento remoto mineral basearam-se em representações tabulares dos dados espectrais, nas quais cada pixel ou amostra é descrito como um vetor de atributos derivados das bandas disponíveis. Modelos clássicos (Random Forest, Support Vector Machines) e redes neurais rasas (Multi-Layer Perceptrons) foram amplamente empregados para classificação litológica e identificação de assinaturas minerais, demonstrando capacidade de modelar relações não lineares entre variáveis espectrais. Porém, essa abordagem carrega uma limitação estrutural fundamental: ao descartar a componente espacial das imagens, ignora informações críticas para interpretação geológica, já que processos de alteração mineral, zonas de contato litológico e padrões geomorfológicos se manifestam como estruturas contínuas e texturas distribuídas no espaço.

&emsp;&emsp; Essa lacuna motivou a exploração de abordagens baseadas em visão computacional para dados de sensoriamento remoto. Redes neurais convolucionais (CNN) permitem aprender automaticamente padrões espaciais e espectrais a partir de janelas de imagem, preservando relações de vizinhança entre pixels e capturando estruturas que representações tabulares não conseguem expressar. Para prospecção mineral, essa capacidade é particularmente valiosa: permite identificar padrões sutis associados a zonas de alteração ou assinaturas espectrais distribuídas espacialmente, potencialmente ampliando a generalização de modelos em cenários com dados limitados. É nesse contexto que o presente trabalho se situa: testando empiricamente se a preservação de informação espacial em chips multiespectrais resulta em vantagem competitiva sobre abordagens puramente tabulares.

&emsp;&emsp; A proposta do presente trabalho assenta-se na hipótese de que a incorporação de informação espacial por meio de chips multiespectrais rotulados pode permitir a extração automática de características relevantes, fornecendo uma base mais robusta para modelagem preditiva em tarefas de mapeamento prospectivo de elementos de terras raras. Este trabalho contribui através de três vertentes complementares: (1) estabelecimento de um pipeline reprodutível de ciência de dados geoespaciais, desde aquisição até geração supervisionada de chips, com protocolos rigorosos de controle de data leakage; (2) comparação sistemática entre modelos tabulares (baselines clássicos e MLP) e visão computacional (CNN), avaliando qual abordagem melhor captura características espectrais-espaciais para prospectiva; e (3) estudo de ablação controlado para isolar impacto de decisões arquiteturais e regularização em cenários de dados limitados.
...
Para concretizar a hipótese central, estabelecem-se os seguintes objetivos específicos:

• Construir protocolo robusto de engenharia de atributos espectrais, investigando transformações e combinações de bandas ASTER para realce de minerais de alteração hidrotermal (argilas, óxidos e carbonatos), indicadores frequentes de sistemas mineralizados.

• Desenvolver e comparar arquiteturas de visão computacional (CNN) com baselines supervisionados clássicos (SVM, Random Forest, Regressão Logística) e tabular (MLP), avaliando qual abordagem captura melhor as características espectrais-espaciais relevantes para prospectiva.

• Implementar validação estratificada e rigorosa com isolamento de conjunto de teste, utilizando métricas robustas ao desbalanceamento de classes (F1-score, acurácia balanceada, ROC-AUC) para avaliação confiável de desempenho.

• Produzir ranking de áreas com maior potencial prospectivo, demonstrando aplicabilidade prática do pipeline para apoio à tomada de decisão em exploração geológica de campo e priorização de campanhas futuras.

&emsp;&emsp; Na seção seguinte, revisamos o estado da arte em sensoriamento remoto mineral e aprendizado de máquina para prospecção, destacando as lacunas metodológicas em reprodutibilidade, integração de informação espacial e validação rigorosa que este trabalho pretende abordar.
